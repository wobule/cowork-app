...
import { getAgent, getTaskById } from '@/lib/db';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const agent = await getAgent(body.agent_id) as Record<string, unknown> | null;
    if (!agent) {
    ...
    if (agent.current_task_id) {
      currentTask = await getTaskById(agent.current_task_id as string);
    }
